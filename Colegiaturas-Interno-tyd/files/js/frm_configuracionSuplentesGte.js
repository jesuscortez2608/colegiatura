$(function(){	
	...
	$("#txt_iduempleado").prop("disabled", true);
	...
	var nivel_jerarquico_usuario = 0;
	...
	function ObtenerDatosUsuario(){
		...
		.done(function(data){
			...
			$("#txt_iduempleado").val(dataJson.usuario);
			$("#txt_nomempleado").val(dataJson.nombre);
			$("#txt_iducentro").val(dataJson.num_centro);
			$("#txt_nomcentro").val(dataJson.nom_centro);
			$("#txt_idupuesto").val(dataJson.num_puesto);
			$("#txt_nompuesto").val(dataJson.nom_puesto); 								
			$("#txt_iduempleado2").focus();				
			$("#txt_FechaIni").prop('disabled',false);
			$("#txt_FechaFin").prop('disabled',false);
			
			obtenerJerarquiaPuesto($("#txt_idupuesto").val(), function(nivel_jerarquico){
				console.log("Nivel jerarquico usuario: " + nivel_jerarquico);
				nivel_jerarquico_usuario = nivel_jerarquico;
				if (nivel_jerarquico_usuario==0){
					//showalert(LengStrMSG.idMSG128, "", "gritter-success");
					showalert("Es necesario configurar jerarquía del gerente", "", "gritter-success");
				}
			});			
		});
	}
	...
	//OBTENER JERARQUIA
	function obtenerJerarquiaPuesto(idu_puesto, callback){
		$.ajax({type:'POST',
			url:'ajax/json/json_fun_obtener_jerarquias_puestos.php',
			data:{'order_column':'value'
				, 'order_type' : ''
				, 'search_field' : 'idu_puesto'
				, 'search_value' : idu_puesto
			...
			success:function(data){
				var dataJson = JSON.parse(data);
				if (dataJson.datos[0] != undefined && dataJson.datos[0].nivel_jerarquico != undefined) {
					callback(dataJson.datos[0].nivel_jerarquico);
				} else {
					callback(0);
				}
			},
	...
	{		
		jQuery("#gridSuplentes-table").jqGrid({
			url:'ajax/json/json_fun_obtener_bitacora_suplentesGte.php?session_name=' +Session,
			//'ajax/json/json_fun_obtener_bitacora_suplentes.php',
			datatype: 'json',
			mtype: 'GET',
			colNames:LengStr.idMSG84,
				colModel:[
				...
				{name:'idu_id',					index:'idu_id', 				width:50, 	sortable: false,	align:"left",	fixed: true,  	hidden:true},
				...
				{name:'idu_centro',				index:'idu_centro', 			width:220, 	sortable: false,	align:"left",	fixed: true},
				{name:'idu_suplente',			index:'idu_suplente', 			width:100, 	sortable: false,	align:"left",	fixed: true, 	hidden:true },
				{name:'nom_suplente',			index:'nom_suplente', 			width:300, 	sortable: false,	align:"left",	fixed: true},
				{name:'idu_centro_suplenete',	index:'idu_centro_suplenete', 	width:220, 	sortable: false,	align:"left",	fixed: true},
				{name:'idu_empleado_registro',	index:'idu_empleado_registro', 	width:300, 	sortable: false,	align:"left",	fixed: true},
				{name:'fec_registro',			index:'fec_registro', 			width:100, 	sortable: false,	align:"left",	fixed: true},
				{name:'fec_inicial',			index:'fec_inicial', 			width:80, 	sortable: false,	align:"left",	fixed: true},
				{name:'fec_final',				index:'fec_final', 				width:80, 	sortable: false,	align:"left",	fixed: true},				
				{name:'opc_indefinido',			index:'opc_indefinido', 		width:100, 	sortable: false,	align:"left",	fixed: true, 	hidden:true},
				{name:'nom_indefinido',			index:'nom_indefinido', 		width:100, 	sortable: false,	align:"center",	fixed: true, 	hidden:true},
				{name:'expirado',				index:'expirado', 				width:100, 	sortable: false,	align:"center",	fixed: true, 	hidden:true},
				{name:'fec_baja',				index:'fec_baja', 				width:100, 	sortable: false,	align:"left",	fixed: true},
				{name:'cancelado',				index:'cancelado', 				width:100, 	sortable: false,	align:"center",	fixed: true, 	hidden:true/*,formatter: rowColorFormatter*/},
				{name:'marcado',				index:'marcado', 				width:100, 	sortable: false,	align:"left" ,	fixed: true,	hidden: true}
			],
			...
			loadComplete: function (datos) {
				...
				if (!datos.rows){													
					return;
						...
						for(var ii=0; ii < datos.rows.length; ii++ )
						{
							if(datos.rows[ii].cell[16] == 1) //campo cancelado del grid 
							{
		...
		barButtongrid({
			...
			},
			{
				...
				click: function(event)
				{
					//console.log($("#gridSuplentes-table").find("tr").length - 1);
					...
					else
					{
						...
						var dataJson = JSON.parse(sjson);
						//console.log('longitud='+dataJson.length)
						if(dataJson.length==0)
						{
	...
	function rowColorFormatter(cellValue, options, rowObject) {
		console.log ('celda='+cellValue+' options='+options.rowId);
		if (cellValue == "1")
	...
	{
		console.log('ConsultarSuplentes');
		var numEmp=0;
		...
		{ url: 'ajax/json/json_fun_obtener_bitacora_suplentesGte.php?session_name=' +Session}).trigger("reloadGrid"); 
	...
	*/
	...
	function LimpiarDatosSuplente(opc){
		console.log('Limpiar Datos Suplente');
		$("#txt_nomempleado2").val("");
		$("#txt_iducentro2").val("");
		$("#txt_nomcentro2").val("");
		$("#txt_idupuesto2").val("");
						...
						else
						{
							$.ajax({type: "POST",
								...
								success:function(data){
									...
									{
										console.log('es Gerente='+dataJsonP.estado);
										if(dataJsonP.tipo==1)// es gerente
										{	
											//showalert("Es gerente", "", "gritter-warning");
											/*$.ajax({type: "GET", 
											url:'ajax/json/json_fun_obtener_jerarquias_puestos.php?session_name='+Session+'&idPuestoGte='+$("#txt_idupuesto").val()+'&idPuestoSte='+$("#txt_idupuesto2").val(),
												data: { 												
												}
											})
											.done(function(data){
									...
									// else
									// {//error al consultar
										// showalert(LengStrMSG.idMSG88+" el puesto", "", "gritter-error");
									// } 
								},
								error:function onError(){},
								complete:function(){},
								...
								success:function(data){
									...
									if (dataJsonP.estado==0)
									{
										if(dataJsonP.tipo==1)// es gerente
										{
											...
											obtenerJerarquiaPuesto(dataJson.idu_puesto, function(nivel_suplente) {
												console.log("Suplente: " + nivel_suplente);
												console.log("Usuario: " + nivel_jerarquico_usuario);
												//nivel_suplente=0;
												if (nivel_suplente==0){
													//showalert(LengStrMSG.idMSG128, "", "gritter-success");
													showalert("Es necesario configurar jerarquía del suplente", "", "gritter-success");
													$("#txt_iduempleado2").val("");
													$("#txt_nomempleado2").val("");
													$("#txt_iducentro2").val("");
													$("#txt_nomcentro2").val("");
												...
												if (nivel_suplente < nivel_jerarquico_usuario) {
													showalert("Unicamente se acepta como suplentes los puestos de su mismo nivel jerarquico o superior", "", "gritter-warning");
													$("#txt_iduempleado2").val("");
													$("#txt_nomempleado2").val("");
													$("#txt_iducentro2").val("");
													$("#txt_nomcentro2").val("");
...
//--BOTON GUARDAR
	$('#btn_guardar').click(function(event)
	{
		...
		//console.log($("#txt_nomempleado2").val().length);
		console.log('nEditar='+nEditar)
		//return;
		if(nEditar == 0)
		{
			id = 0;
		...
		else if($("#txt_iduempleado2").val().length <8)
		{
			//showmessage('Proporcione el suplente', '', undefined, undefined, function onclose(){});
			showalert("Favor de agregar número de empleado válido", "", "gritter-info");
			return;
		}
...
//--GUARDAR BITACORA	
	...
	{
		console.log("recorrer grid modificar");
		var arr_cancelar = [];
		for(var i = 0; i < $("#gridSuplentes-table").find("tr").length - 1; i++) {
			var jdata = $("#gridSuplentes-table").jqGrid('getRowData',i);
			if ( jdata.marcado == 1 ) {
