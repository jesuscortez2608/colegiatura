    ...
    if($estado != 0)
     {
		...
		try
	    {
			...
			// echo ("{CALL fun_estatus_facturas_externos()}");		
			// exit();
	        $cmd->setCommandText("{CALL fun_estatus_facturas_externos()}");
			...
			foreach ($ds as $fila) 
	        {
	        	...
	        	$json->datos[$i]->idu_estatus = $fila['iestatus'];
	        	$json->datos[$i]->nom_estatus = trim($fila['snombreestatus']);
	            $i++; 
	        }
	    }
	    catch(exception $ex)
