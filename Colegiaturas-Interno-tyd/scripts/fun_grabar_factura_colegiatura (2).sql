...
CREATE OR REPLACE FUNCTION public.fun_grabar_factura_colegiatura(idempleado integer, ibeneficiarioext integer, ifactura integer, ieditar integer, ccomentario text, cxmlrecibo character varying, xpdfcarta character varying, iempresa integer, icentro integer, itipodoc integer, numgte Integer, nomgte character varying, OUT ifoliofactura integer, OUT cfactura character varying)
...
DECLARE     
	...
	Modifica					   : Ruth Martínez Hernández 90286617
	Descripción del cambio         : Automatización de autorización de gerente. Se pasa a estatus ""En Proceso"" en cuanto se sube la factura
									 y se agrega a bit_estatus_factura_colegiaturas.
									 
	Modifica					   : Luis Eduardo Hernandez Jimenez 90244610
	Descripción del cambio         : Se modifica guardado de bitacora para almacenar numero y nombre de gerente, de igual manera se usa para la
									automatizacion de gerente.
	Ejemplo                        : 
...
--raise notice 'ff-- :% % iFolioFactura %', idEmpleado, iFolioFactura, idfactura;
			...
			/*AUTOMATIZACIÓN AUTORIZA GERENTE*/
			UPDATE mov_facturas_colegiaturas m
			SET idu_estatus = 1, fec_marco_estatus =  NOW(), emp_marco_estatus = numgte
			WHERE m.idu_empleado = idEmpleado
			AND m.idfactura = iFolioFactura;

			-- INSERTAR BITÁCORA
			INSERT into bit_estatus_factura_colegiaturas (idfactura, numemp, idu_ultimo_estatus, fec_autorizo_gerente, numemp_autorizo_gerente, nom_autorizo_gerente) 
			VALUES (iFolioFactura, idEmpleado, 1, current_date, numgte, nomgte);
			/*AUTOMATIZACIÓN AUTORIZA GERENTE*/
...
GRANT EXECUTE ON FUNCTION fun_grabar_factura_colegiatura(idempleado integer, ibeneficiarioext integer, ifactura integer, ieditar integer, ccomentario text, cxmlrecibo character varying, xpdfcarta character varying, iempresa integer, icentro integer, itipodoc integer, numgte Integer, nomgte character varying) TO syspersonal;
GRANT EXECUTE ON FUNCTION fun_grabar_factura_colegiatura(idempleado integer, ibeneficiarioext integer, ifactura integer, ieditar integer, ccomentario text, cxmlrecibo character varying, xpdfcarta character varying, iempresa integer, icentro integer, itipodoc integer, numgte Integer, nomgte character varying) TO postgres;