drop function if exists fun_generar_recibo_bancoppel(integer, date);
drop type if exists type_recibo_bancoppel;

CREATE TYPE type_recibo_bancoppel AS
   (linea character varying(219));
   
CREATE OR REPLACE FUNCTION fun_generar_recibo_bancoppel(integer, date)
  RETURNS SETOF type_recibo_bancoppel AS
$BODY$
DECLARE
	/*
	Fecha                          : 19/01/2017
	...
	Módulo                         : Generación de recibo para BanCoppel
	Ejemplo                        : 
		select linea from fun_generar_recibo_bancoppel(95194185::integer, '20170310'::date);
	...
	returnrec type_recibo_bancoppel;
BEGIN
	...
	-- Nombre del archivo generado para bancoppel (0012013112204.dat)
	cNombreArchivoGenerado := '001' || to_char(dFecha::date,'yyyy') || to_char(dFecha,'mm') || to_char(dFecha,'dd') || '04.dat';
	insert into tmp_archivo (linea) values ('          REPORTE DE MOVIMIENTOS GENERADOS A PAGO CON TARJETA BANCOPPEL: ' || cNombreArchivoGenerado);
	insert into tmp_archivo (linea) values ('');
