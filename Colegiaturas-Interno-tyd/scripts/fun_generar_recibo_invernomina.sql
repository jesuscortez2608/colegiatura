drop function if exists fun_generar_recibo_invernomina(integer, date);
drop type if exists type_recibo_invernomina;

CREATE TYPE type_recibo_invernomina AS
   (linea character varying(219));


CREATE OR REPLACE FUNCTION fun_generar_recibo_invernomina(integer, date)
  RETURNS SETOF type_recibo_invernomina AS
$BODY$
DECLARE
	/*
	Fecha                          : 19/01/2017
	...
	Descripción del funcionamiento : Obtiene las líneas que conformarán el recibo de pagos de
	                                 colegiaturas para Invernomina
	Descripción del cambio         : No aplica
	Sistema                        : Colegiaturas
	Módulo                         : Generación de recibo para Invernomina
	Ejemplo                        : 
		select linea from fun_generar_recibo_invernomina(95194185::integer, '20170310'::date);
		update mov_generacion_pagos_colegiaturas set clv_rutapago = 4;
		update mov_detalle_generacion_pagos_colegiaturas set clv_rutapago = 4;
	------------------------------------------------------------------------------------------------------ */
	...
	returnrec type_recibo_invernomina;
BEGIN
	...
	-- Nombre del archivo generado para Invernomina (0012013112204.dat)
	cNombreArchivoGenerado := '001' || to_char(dFecha::date,'yyyy') || to_char(dFecha,'mm') || to_char(dFecha,'dd') || '04.dat';
	insert into tmp_archivo (linea) values ('          REPORTE DE MOVIMIENTOS GENERADOS A PAGO CON TARJETA INVERNOMINA: ' || cNombreArchivoGenerado);
	insert into tmp_archivo (linea) values ('');
