drop function if exists fun_generar_recibo_bancomer(integer, date);
drop type if exists type_recibo_bancomer;

CREATE TYPE type_recibo_bancomer AS
   (linea character varying(219));

CREATE OR REPLACE FUNCTION fun_generar_recibo_bancomer(integer, date)
  RETURNS SETOF type_recibo_bancomer AS
$BODY$
DECLARE
	/*
	Fecha                          : 19/01/2017
	...
	Módulo                         : Generación de recibo para Bancomer
	Ejemplo                        : 
		select linea from fun_generar_recibo_bancomer(95194185::integer, '20170310'::date);
		update mov_generacion_pagos_colegiaturas set clv_rutapago = 5;
		update mov_detalle_generacion_pagos_colegiaturas set clv_rutapago = 5;
	------------------------------------------------------------------------------------------------------ */
	...
	returnrec type_recibo_bancomer;
BEGIN
	...
	-- Nombre del archivo generado para bancomer (0012013112204.dat)
	cNombreArchivoGenerado := '001' || to_char(dFecha::date,'yyyy') || to_char(dFecha,'mm') || to_char(dFecha,'dd') || '04.dat';
	insert into tmp_archivo (linea) values ('          REPORTE DE MOVIMIENTOS GENERADOS A PAGO CON TARJETA BANCOMER: ' || cNombreArchivoGenerado);
	insert into tmp_archivo (linea) values ('');
